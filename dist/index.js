...
module.exports = function variables(string, recursive = false) {
    ...
    const activeWorkspace = workspaceFolders?.find((workspace) => absoluteFilePath?.startsWith(workspace.uri.fsPath))?.uri.fsPath;
    ...
    string = string.replace(/\${workspaceFolder:(.*?)}/g, function (_, name) {
        return (workspaceFolders?.find((workspace) => workspace.name === name)?.uri
            .fsPath ?? "");
    });
    // ${workspaceFolderBasename} - your-project
    string = string.replace(/\${workspaceFolderBasename}/g, workspace?.name ?? "");
    // ${workspaceFolderBasename:name} - your-project2
    string = string.replace(/\${workspaceFolderBasename:(.*?)}/g, function (_, name) {
        return (workspaceFolders?.find((workspace) => workspace.name === name)?.name ??
            "");
    });
    ...
    string = string.replace(/\${relativeFile}/g, absoluteFilePath?.substring(activeWorkspace?.length ?? 0) ?? "");
    // ${relativeFileDirname} - folder
    string = string.replace(/\${relativeFileDirname}/g, absoluteFilePath?.substring(activeWorkspace?.length ?? 0, absoluteFilePath?.lastIndexOf(os.platform() === "win32" ? "\\" : "/")) ?? "");
    // ${fileBasename} - file.ext
    string = string.replace(/\${fileBasename}/g, absoluteFilePath?.split("/")?.pop() ?? "");
    // ${fileBasenameNoExtension} - file
    string = string.replace(/\${fileBasenameNoExtension}/g, absoluteFilePath?.split("/").pop()?.split(".")?.shift() ?? "");
    // ${fileDirname} - /home/your-username/your-project/folder
    string = string.replace(/\${fileDirname}/g, absoluteFilePath?.split("/")?.slice(0, -1)?.join("/") ?? "");
    // ${fileExtname} - .ext
    string = string.replace(/\${fileExtname}/g, absoluteFilePath?.split(".")?.pop() ?? "");
    // ${lineNumber} - line number of the cursor
    string = string.replace(/\${lineNumber}/g, (vscode.window.activeTextEditor
    ...
    string = string.replace(/\${pathSeparator}/g, os.platform() === "win32" ? "\\" : "/");
